...
class MicrophoneProducer:
    ...
    def start(self):
        ...
        self.producer_thread = threading.Thread(
            ...
            daemon=True,
            name="MicrophoneProducer"
        )
        self.producer_thread.start()
        
        print(f"[生产者] 已启动 (采样率: {self.sample_rate}Hz, 块大小: {self.chunk_size})")
...
class SpeechRecognitionConsumer:
    ...
    def _init_recognizer(self):
        ...
        if self.recognition_engine == 'whisper':
            ...
            try:
                ...
                print("[消费者] 错误: 未安装 SpeechRecognition。运行: pip install SpeechRecognition")
                self.recognizer = None
    ...
    def start(self):
        ...
        self.consumer_thread = threading.Thread(
            ...
            daemon=True,
            name="SpeechRecognitionConsumer"
        )
    ...
    def _process_audio(self, audio_message: dict):
        ...
        if self.debug:
            print(f"[{timestamp:.2f}] 音量: {volume:.2f}, 说话中: {self.is_speaking}, padding: {self.in_padding}")
        ...
        if is_speech:
            ...
            if self.is_speaking and not self.in_padding:
                ...
                if silence_duration >= self.speech_timeout:
                    ...
                    if self.debug:
                        print(f"[消费者] 静音 {silence_duration:.2f}秒，进入padding阶段")
                ...
                if padding_duration >= self.silence_padding:
                    ...
                    self.speech_buffer.extend(self.padding_buffer)
                    if self.debug:
                        print(f"[消费者] padding {padding_duration:.2f}秒结束，触发识别")
    ...
    def _recognize_speech_buffer(self):
        ...
        if duration < self.min_speech_duration:
            if self.debug:
                print(f"[消费者] 语音时长 {duration:.2f}秒 < 最小时长 {self.min_speech_duration}秒，跳过识别")
            return
    ...
    def main():
        ...
        try:
            ...
            while True:
                ...
                print(f"\r队列状态: {status['queue_usage']} | 丢弃: {status['dropped_count']}", end='', flush=True)
